...
import { ColumnDef, FlexRenderDirective, VisibilityState, createAngularTable, flexRenderComponent, getCoreRowModel } from '@tanstack/angular-table';
...
@Component({
  ...
  imports: [FlexRenderDirective, HlmTableImports, HlmDropdownMenuImports, HlmButtonImports, NgIcon, HlmSpinnerImports, HlmEmptyImports],
  templateUrl: './accounts-table.component.html',
})
export class AccountsTableComponent {
  protected readonly managementService = inject(ManagementService);

  private readonly _columnVisibility = signal<VisibilityState>(JSON.parse(localStorage.getItem(STORAGE_COLUMNS_ITEM) ?? "{}"));
  ...
  constructor() {
    ...
    effect(() => {
      localStorage.setItem(STORAGE_COLUMNS_ITEM, JSON.stringify(this._columnVisibility()))
    })
  }
